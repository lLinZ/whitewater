...
import { Plus, ShoppingCart, ArrowUp, ArrowDown, Trash2, Receipt, ScrollText } from 'lucide-react';
...
export default function MarketIndex({ trips }: { trips: ShoppingTrip[] }) {
    const { rates, features, auth } = usePage<PageProps>().props;
    ...
    return (
        <AppLayout title="Mercado" subtitle="Compras y conversión">
            ...
            <button
                onClick={startTrip}
                className={`mt-3 flex w-full items-center justify-center gap-2 rounded-3xl bg-gradient-to-br py-4 font-semibold text-white shadow-card active:scale-[0.99] ${accent(auth.user.color).gradient}`}
            >
                <Plus size={20} /> Empezar mercado
            </button>

            {features?.invoiceScan && <ScanInvoiceButton label="Escanear una factura" className="mt-2" />}
                                            ...
                                            <p className="text-xs text-default-400">
                                                ...
                                                {t.receipts.length > 0 && (
                                                    <span className="ml-1 whitespace-nowrap">
                                                        · <ScrollText size={11} className="inline" /> {t.receipts.length}
                                                    </span>
                                                )}
